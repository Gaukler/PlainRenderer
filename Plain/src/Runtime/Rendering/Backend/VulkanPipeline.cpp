...
VkPipeline createVulkanGraphicsPipeline(
    ...
    const VkPipelineMultisampleStateCreateInfo  multisamplingState  = createDefaultMultisamplingInfo();
    const VkPipelineDepthStencilStateCreateInfo depthStencilState   = createDepthStencilState(desc.depthTest);
    ...
    const VkPipelineDynamicStateCreateInfo          dynamicStateInfo    = createDynamicStateInfo(dynamicStates);
    const VkPipelineInputAssemblyStateCreateInfo    inputAssemblyState  = createInputAssemblyInfo(desc.rasterization.mode);
    const VkPipelineViewportStateCreateInfo         viewportState       = createDynamicViewportCreateInfo();
    ...
    const VkPipelineVertexInputStateCreateInfo              vertexInputInfo = createPipelineVertexInputStateCreateInfo(vertexBinding, attributes);
    ...
    VkPipeline pipeline;
    const auto result = vkCreateGraphicsPipelines(vkContext.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline);
