...
ShaderReflection performShaderReflection(const GraphicPassShaderSpirV& shaderCode) {
    ...
    if (shaderCode.tessCtrl.has_value()) {
        assert(shaderCode.tessEval.has_value());
        layoutFromSpirv(shaderCode.tessCtrl.value(), VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT, &reflection);
        layoutFromSpirv(shaderCode.tessEval.value(), VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, &reflection);
    }
